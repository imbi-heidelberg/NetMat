---
title: "Enrichment through Weighting"
...
dataprep <- data.prep(arm.data = nsclc_data,
                      ...
                      varname.s = "study")
```
## Extension of the model via Enrichtment through weighting (Proctor et al. 2022)
...
Analogous to the basic model construction laid out for the meta-regression you can now also employ enrichment-based methods along the work of Efthimiou et. al. (2017) for the integration of randomized and non-randomized evidence in network meta-analysis.
...
focus may shift onto this specific subpopulation in subsequent research. The enrichment through weighting approach developed by Proctor et al. allows to synthesize multiple studies with these characteristics and take their respective proportion of the regarding subpopulations of interest into account.
...
nma.regplot(enrichment_covariate_results)
```

```{r, echo=TRUE, fig.width=7, fig.height=4}
nma.regplot(enrichment_prior_results)
```
